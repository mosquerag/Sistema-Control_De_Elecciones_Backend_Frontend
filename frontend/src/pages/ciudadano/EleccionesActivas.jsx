...
const EleccionesActivas = () => {
  ...
  const getEstadoCiudadano = (eleccion) => {
    const activa = isEleccionActiva(eleccion.fechaInicio, eleccion.fechaFin);
    const yaVoto = votosMap[eleccion._id] ?? false;
    if (yaVoto && !activa) return "votada_finalizada";
    if (yaVoto && activa) return "votada";
    if (!yaVoto && activa) return "pendiente";
    return "finalizada";
  };
  ...
  const eleccionesFiltradas = elecciones.filter((eleccion) => {
    ...
    const matchEstado =
      filterEstado === "todos" || getEstadoCiudadano(eleccion) === filterEstado;
    ...
    <>
      ...
      <FilterBar
        ...
        firstSelectOptions={[
          ...
          { value: "pendiente", label: "Pendientes por votar" },
          { value: "votada", label: "Votadas (activas)" },
          { value: "votada_finalizada", label: "Votadas (finalizadas)" },
          { value: "finalizada", label: "Finalizadas sin votar" },
        ]}
              ...
              <div
                ...
                className={`bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm
                  ...
                  border-2 ${
                    ...
                    yaVoto && !activa
                      ? "border-red-400 dark:border-red-500 !bg-red-300 dark:!bg-red-900/40" // votó + finalizada → rojo
                      : yaVoto && activa
                        ? " !bg-green-300 dark:!bg-green-900/40 border-green-400 dark:border-green-600 " // votó + activa → verde
