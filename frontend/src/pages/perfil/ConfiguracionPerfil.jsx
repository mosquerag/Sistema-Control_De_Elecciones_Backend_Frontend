...
// ─────────────────────────────────────────────────────────────
const getCamposPermitidos = (rol, requiereCedula) => {
  const base = ["nombre", "email", "telefono", "direccion", "municipio"];
  if (requiereCedula) base.push("cedula");
...
export function ConfiguracionPerfil({ onClose }) {
  ...
  // ── Google ─────────────────────────────────────────────────
  const esGoogle =
    user?.esGoogleAuth === true ||
    user?.proveedor === "google" ||
    user?.esGoogle === true;
  // Google no entrega cédula: si es admin de Google y aún no la tiene
  // registrada, se le pide aquí (aplica también a cuentas ya existentes)
  const requiereCedula = esGoogle && user?.rol === "admin" && !user?.cedula;
  // Google a veces no trae región en el locale — se pide el país manualmente
  const requierePais = esGoogle && user?.rol === "admin" && !user?.pais;
  const camposPermitidos = getCamposPermitidos(user?.rol, requiereCedula);
  const listaPaises = requierePais ? Country.getAllCountries() : [];
  ...
  const validarPerfil = () => {
    ...
    if (requiereCedula) {
      if (!formData.cedula?.trim()) e.cedula = "La cédula es obligatoria";
      else if (!/^\d{11}$/.test(formData.cedula.trim()))
        e.cedula = "La cédula debe tener 11 dígitos";
    }
    if (requierePais && !formData.pais?.trim())
      ...
      async () => {
        try {
          ...
          if (JSON.stringify(formData) !== JSON.stringify(originalData)) {
            ...
            // Admin de Google completando la cédula que Google no entrega
            if (requiereCedula) {
              dataToSend.cedula = formData.cedula.trim();
            }
            // Admin de Google completando el país cuando el locale no bastó
            if (requierePais) {
              dataToSend.pais = formData.pais;
            }
            if (requiereCedula || requierePais) {
              dataToSend.perfilPendiente = false;
            }
      ...
      <div className="p-6 overflow-y-auto max-h-[60vh]">
        ...
        {activeTab === "perfil" && (
          <form onSubmit={handleSubmitPerfil} className="space-y-4">
            {/* ── Foto de perfil ─────────────────────────── */}
            <div className="flex flex-col items-center gap-3 pb-4 border-b border-gray-200 dark:border-slate-700">
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Cédula — editable solo si Google no la trajo, si no, solo lectura */}
              {requiereCedula ? (
                <div>
            ...
            {/* País — solo se pide si Google no lo pudo detectar */}
            {requierePais ? (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="pais">
                  ...
                  <select
                    ...
                    value={formData.pais}
                    onChange={handleChange}
                    className="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
              ...
              user?.pais && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="paisSolo">
                      País{" "}
                      <span className="text-xs text-gray-400">
                        (No modificable)
                      </span>
                    </Label>
                    <Input id="paisSolo" value={user.pais} disabled />
                  </div>
                </div>
              )
            )}
