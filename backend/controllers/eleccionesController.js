...
export const updateEleccion = async (req, res) => {
  try {
    // ── Si la elección ya tiene votos, proteger los campos críticos ──
    // (fechas y tipo no deben poder cambiarse después de que la gente
    // ya votó bajo esas condiciones — evita manipular resultados)
    ...
    if (intentaCambiarCritico) {
      ...
      if (totalVotos > 0) {
        return res.status(400).json({
          success: false,
          message:
            "Esta elección ya tiene votos registrados. No se pueden modificar las fechas ni el tipo de elección.",
